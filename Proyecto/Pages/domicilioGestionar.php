 ...
 if (isset($_POST['eliminar'])){
    ...
    if ($consulta) {
      ...
      if ($valor>0){
        ...
        $sql="DELETE FROM domicilio WHERE id_domicilio='$_REQUEST[idDomicilio]'";
        // Se ejecuta el comando
        $consulta=mysqli_query($conexion,$sql);
        // Alerta en caso de ser eliminado correctamente
        echo '<script> alert("Domicilio eliminado");</script>';
      }else{
        // alerta en caso de que el rfc no exista
        echo '<script> alert("ERROR: El domicilio no existe");</script>';
      }
    }else{
...
if (isset($_POST['modificar'])) {
    ...
    if ($consulta) {
      ...
      if ($valor>0){
        ...
        header ("Location: modificarDomicilio.php?var_pin=".($id));
      }else{
